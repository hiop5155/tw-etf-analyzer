...
| **目標試算** | 正推(目標 → 月投)+ 反推(月支出 → 資產,4% 法則 + SWR 對照)；支援多檔現有持股自動查價與逐檔終值試算 |
...
| **多檔比較** | 2–5 檔標準化走勢 + 風險指標 + 月報酬相關性熱圖；可設定自己的比較用定投金額 |
| **股利歷史** | 歷年配息、殖利率雙軸圖、明細；使用自己的查詢標的 |
...
- `績效分析` 分頁自己管理 `股票代號` 與 `每月定期定額`
- `目標試算` 分頁自己管理 `試算標的` 與 `目前持股明細`
- `多檔比較` 分頁自己管理比較用 `每月定期定額`
- `股利歷史` 分頁自己管理查詢 `股票代號`
- 各分頁輸入互不干擾,都會寫入瀏覽器 localStorage
...
目前包含:
- 績效分析的 `股票代號`、`每月定期定額`
- 目標試算的 `試算標的`、`目標金額`、`投資年限`、`目前持股明細`
- 多檔比較的比較代號與比較用 `每月定期定額`
- 股利歷史的查詢標的
- 退休模擬 / 提領追蹤 / 側邊欄的既有設定

`目標試算` 的 `目前持股明細` 直接用表格編輯:
- 使用者可直接新增列、刪除列、修改 `股票代號` 與 `股數`
- `最新價格`、`歷史年化報酬 %`、`市值` 自動查詢且不可手動修改
- 編輯代號或股數後會自動重算現值與 n 年後終值
